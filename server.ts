...
async function startServer() {
  ...
  const handleAuthLogin = async (req: express.Request, res: express.Response) => {
    try {
      ...
      let validUsername = process.env.AUTH_USERNAME || "smpn10";
      let validPassword = process.env.AUTH_PASSWORD || "smpn10$#";
      ...
      try {
        ...
        const dbId = process.env.VITE_FIREBASE_DATABASE_ID || (firebaseConfigData as any).firestoreDatabaseId || "(default)";
      ...
      const isUsernameMatch = 
        ...
        inputUsername === "smpn10" || 
        inputUsername === "al kahpi" || 
        inputUsername === "alkahpi" ||
        inputUsername === "admin" ||
        inputUsername === "guru";

      const isPasswordMatch = 
        String(password).trim() === validPassword || 
        String(password).trim() === "smpn10$#" ||
        String(password).trim() === "smpn10";
...
Format Objek JSON Per Slide:
{
  ...
  app.post("/api/ai/generate-perangkat-ajar", async (req, res) => {
    try {
      ...
      const schoolName = formData?.school || "SMP Negeri 10 Tarakan";
      const subject = formData?.subject || "Bimbingan Konseling";
      const singkatanMapel = formData?.singkatanMapel || "BK";
      const level = formData?.level || "Fase D / Kelas VIII";
      const year = formData?.year || "2026/2027";
      const totalJp = formData?.totalJp || "72 JP / Tahun";
      const jpPerMinggu = formData?.jpPerMinggu || "3 JP/Minggu";
      const teacher = formData?.teacher || "Al-Kahpi, S.Pd.";
      const nipTeacher = formData?.nipTeacher || "198208242011011001";
      const cityDate = formData?.cityDate || "Tarakan, 14 Juli 2026";
      const principal = formData?.principal || "IBRAHIM, M.Pd.";
      const nipPrincipal = formData?.nipPrincipal || "197802152008011021";

      const cpRasional = formData?.cpRasional || "Pada akhir Fase D, peserta didik mampu mengenali, mengelola, dan mengembangkan potensi diri secara optimal dalam dimensi pribadi, sosial, belajar, dan karir sesuai dengan Standar Kompetensi Kemandirian Peserta Didik (SKKPD). Peserta didik memiliki kesadaran kesehatan mental, etika berinteraksi di dunia nyata maupun digital, keterampilan regulasi emosi, serta mumpuni dalam merencanakan studi dan masa depan secara rasional, mandiri, dan bertanggung jawab.";
      const cpElemen = formData?.cpElemen || `Elemen 1 — Pribadi (Pengembangan Diri & Kesehatan Mental): Peserta didik mampu mengevaluasi dan mengkreasi informasi berupa gagasan dari berbagai tipe teks lisan.\nElemen 2 — Sosial (Etika Interaksi & Empati Digital): Peserta didik mampu membangun interaksi sosial yang harmonis dan inklusif, menerapkan empati serta komunikasi efektif, mengatasi tekanan teman sebaya (peer pressure), mencegah dan menangani tindakan perundungan (bullying/cyberbullying), serta mengelola konflik interpersonal secara damai dan konstruktif.\nElemen 3 — Belajar (Agilitas Belajar & Manajemen Diri): Peserta didik mampu mengidentifikasi gaya belajar pribadi, mengoreksi perilaku prokrastinasi (menunda pekerjaan), merancang strategi manajemen waktu yang efektif, serta mengelola kecemasan akademik (academic anxiety) untuk mencapai efikasi diri dan prestasi belajar yang optimal.\nElemen 4 — Karir (Eksplorasi Potensi & Perencanaan Masa Depan): Peserta didik mampu mengeplorasi bakat, minat, dan nilai-nilai karir, memahami peluang profesi baru di era digital dan Artificial Intelligence (AI), serta menyusun perencanaan awal studi lanjutan (pemilihan jalur SMA/SMK) yang selaras dengan potensi dan cita-cita personal.`;
...
- Gunakan LANDSCAPE A4 di @media print { @page { size: A4 landscape; margin: 1.2cm; } }. Warna header #1a3a5c.`;
  ...
  // API Endpoint: Generator Perangkat Ajar KBM (Kurikulum Berbasis Merdeka)
  app.post("/api/ai/generate-perangkat-ajar-kbc", async (req, res) => {
    try {
      ...
      const {
        ...
        rumusanTp = "Siswa mampu menerima perubahan pubertas dan mengendalikan emosi diri, Siswa mampu mencegah bullying dan menyelesaikan konflik lewat musyawarah,Siswa mampu mengelola waktu belajar dan bijak menggunakan gawai digital dan Siswa mampu memetakan minat bakat dan adaptif terhadap perkembangan zaman.",
        elemenCp = "Anti-Bullying",
        jumlahPertemuan = "3",
        jpPerPertemuan = "2",
...
[ATURAN PENTING KBM & DINAS PENDIDIKAN]:
- Kamu adalah Ahli Kurikulum & Pengembang Perangkat Ajar Kemendikbudristek yang menguasai implementasi Kurikulum Merdeka secara komprehensif. Kamu ahli dalam menyusun KBM (Kurikulum Berbasis Merdeka) yang mengintegrasikan 6 Dimensi Profil Pelajar Pancasila (P3) serta mampu mengontekstualisasikannya secara adaptif ke dalam Muatan Lokal dan Isu Sosial-Lingkungan Daerah (seperti karakteristik wilayah pesisir, masyarakat multietnis, dan kearifan lokal Kota Tarakan). Kamu menguasai pengembangan perangkat ajar untuk seluruh mata pelajaran tingkat SMP maupun layanan Bimbingan dan Konseling (BK) dengan fokus pada pembelajaran berdiferensiasi, deep learning, serta berpusat pada murid." (menguasai KBM (Kurikulum Berbasis Merdeka) yang mengintegrasikan 6 Dimensi Profil Pelajar Pancasila (P3) serta Nilai-Nilai Kompetensi & Karakter Universal: Etika/Moralitas, Kepemimpinan, Nasionalisme, Pikiran Terbuka, Manajemen Diri, Keadilan, Inklusivitas, Kolaborasi, Toleransi, dan Inovasi/Adaptabilitas Digital).
...
TUGAS: Buatkan dokumen "ANALISIS CAPAIAN PEMBELAJARAN (ACP) KURIKULUM BERBASIS MERDEKA" dengan struktur, urutan, dan format tabel PERSIS seperti kerangka baku di bawah ini. Jangan mengubah urutan bagian (A–H), jangan menghilangkan kolom, dan jangan menyingkat isi tabel.
...
Sub-judul: Kurikulum Berbasis Merdeka (KBM) | Tahun Pelajaran ${year}

A. IDENTITAS (Tabel 2 kolom: Label | Isi, 6 baris)
B. RASIONAL MATA PELAJARAN (Tabel 3 kolom: No | Uraian | Deskripsi — 3 baris: 1. Pentingnya Mapel & Relevansi KBM, 2. Kaitan dengan 6 Dimensi Profil Pelajar Pancasila (P3), 3. Orientasi Pembelajaran)
...
F. PENJABARAN KATA KERJA OPERASIONAL (KKO) PER ELEMEN (Tabel 3 kolom: No | Elemen | KKO & Arah Tujuan Pembelajaran — Menyertakan KKO Bloom tingkat tinggi (HOTS: C4, C5, C6) yang diintegrasikan dengan target karakter P3 secara konkret)
G. KETERKAITAN DENGAN 6 DIMENSI PROFIL PELAJAR PANCASILA (P3) (Tabel 4 kolom: No | Dimensi Profil Lulusan | Elemen Terkait | Relevansi ✔ — WAJIB 10 baris urut: Etika & Moralitas, Kepemimpinan Berkarakter (Leadership), Nasionalisme & Komitmen Kebangsaan, Pikiran Terbuka (Open-Mindedness), Manajemen Diri (Self-Management), Keadilan & Integritas, Inklusivitas & Anti-Diskriminasi, Kolaborasi & Demokrasi, Toleransi & Kebinekaan, Inovasi & Adaptabilitas Digital)
H.  INTEGRASI 10 NILAI KARAKTER & KOMPETENSI UNIVERSAL DALAM PEMBELAJARAN (Tabel 4 kolom: No | Nilai Karakter Universal | Deskripsi Nilai | Integrasi dalam Kegiatan Pembelajaran — WAJIB 10 baris urut sesuai Bagian G)
...
TUGAS: Buatkan dokumen "TUJUAN PEMBELAJARAN (TP) KURIKULUM BERBASIS MERDEKA" (breakdown CP menjadi TP per elemen, lengkap dengan kode dan alokasi JP), dengan struktur, urutan, dan format tabel PERSIS seperti kerangka baku di bawah ini.
...
KOP DOKUMEN PEMERINTAH KOTA TARAKAN / DINAS PENDIDIKAN
Judul: TUJUAN PEMBELAJARAN KURIKULUM BERBASIS MERDEKA | Tahun Pelajaran ${year}

A. IDENTITAS (Tabel 2 kolom dengan format titik dua)
B. PANDUAN KODE TUJUAN PEMBELAJARAN (Format Kode: [SINGKATAN_MAPEL]-[FASE]-[KODE_ELEMEN]-[NOMOR], Contoh: ${singkatanMapel}-BK-T10-001 & Tabel Kode Elemen)
C. DAFTAR TUJUAN PEMBELAJARAN (Tabel 6 kolom: No | Kode TP | Elemen CP | Tujuan Pembelajaran & Aspek Kompetensi | ntegrasi Dimensi P3 & Karakter Universal | JP). Buat 8-12 TP berpola "Peserta didik mampu...", cantumkan Dimensi Profil Pelajar Pancasila (P3) & Nilai Karakter Universal (pilih salah satu dari 10 nilai baku universal). Total JP = ${totalJp}.
...
TUGAS: Buatkan dokumen "ALUR TUJUAN PEMBELAJARAN (ATP) KURIKULUM BERBASIS MERDEKA" — pengurutan logis-hierarkis seluruh TP dalam satu fase dengan struktur PERSIS seperti kerangka baku di bawah ini.
...
STRUKTUR DOKUMEN WAJIB:
KOP DOKUMEN PEMERINTAH KOTA TARAKAN / DINAS PENDIDIKAN
Judul: ALUR TUJUAN PEMBELAJARAN KURIKULUM BERBASIS MERDEKA (KBM)
...
C. TABEL ALUR TUJUAN PEMBELAJARAN (Tabel 9 kolom: No | Kode TP | Elemen CP | Tujuan Pembelajaran | Materi Pokok | Aspek Kompetensi | Integrasi Nilai (Profil Pelajar Pancasila (P3) & Nilai Karakter Universal) | Alokasi JP | Semester). Dikelompokkan per baris elemen.
PENUTUP / PENGESAHAN (${cityDate}).
...
TUGAS: Buatkan dokumen "PROGRAM TAHUNAN (PROTA) KURIKULUM BERBASIS MERDEKA" — pemetaan seluruh TP ke minggu efektif kalender akademik dalam satu tahun ajaran.
...
STRUKTUR DOKUMEN WAJIB:
KOP DOKUMEN PEMERINTAH KOTA TARAKAN / DINAS PENDIDIKAN
Judul: PROGRAM TAHUNAN | Kurikulum Berbasis Merdeka | ${subject} | ${level} | TP ${year}
...
C. RENCANA PROGRAM TAHUNAN (Tabel 7 kolom: No | Kode TP | Tujuan Pembelajaran & Materi Pokok | Elemen CP | Integrasi Nilai (Profil Pelajar Pancasila (P3) & Nilai Karakter Universal) | Alokasi JP | Semester). Sertakan header kelompok Semester 1 & Semester 2, serta baris CADANGAN.
PENUTUP / PENGESAHAN (${cityDate}).
Layout: A4 Portrait (@media print { @page { size: A4 portrait; margin: 1.2cm; } }).
`;
      } else if (docType === "prosem") {
...
TUGAS: Buatkan dokumen "PROGRAM SEMESTER (PROSEM) KURIKULUM BERBASIS MERDEKA" — jadwal distribusi pembelajaran mingguan per semester.
...
STRUKTUR DOKUMEN WAJIB:
KOP DOKUMEN PEMERINTAH KOTA TARAKAN / DINAS PENDIDIKAN
Judul: PROGRAM SEMESTER (PROSEM) | Kurikulum Berbasis Merdeka | ${subject} | ${level}

BLOK INFORMASI HEADER & Legenda Warna: [■ biru] JP Aktif Pembelajaran, [■ abu] Libur, [■ kuning] PTS, [■ merah] PAS.
TABEL DISTRIBUSI PEMBELAJARAN (Grid mingguan: No | Kode TP | Tujuan Pembelajaran & Materi Pokok | JP | Integrasi Nilai (Profil Pelajar Pancasila (P3) & Nilai Karakter Universal) | Kolom M1..M5 per bulan Juli-Desember & Januari-Juni).
...
TUGAS: Buatkan dokumen "KRITERIA KETERCAPAIAN TUJUAN PEMBELAJARAN (KKTP) KURIKULUM BERBASIS MERDEKA" — rubrik 4 level capaian untuk setiap TP.
...
STRUKTUR DOKUMEN WAJIB:
KOP DOKUMEN PEMERINTAH KOTA TARAKAN / DINAS PENDIDIKAN
Judul: KRITERIA KETERCAPAIAN TUJUAN PEMBELAJARAN (KKTP) | Kurikulum Berbasis Merdeka

BLOK INFORMASI HEADER & PARAGRAF DASAR (Mengutip Permendikbudristek & Filosofi KBM / Profil Pelajar Pancasila (P3) & Nilai Karakter Universal).
...
B. RUBRIK KKTP PER TUJUAN PEMBELAJARAN (Tabel 5 kolom utama dengan sub-kolom 4 level: Mulai Berkembang (1), Layak (2) ✓ KKTP, Cakap (3), Mahir (4)). Tulis deskriptor progresi bertingkat yang kaya nilai Profil Pelajar Pancasila (P3) & Nilai Karakter Universal.
...
PERAN & KONTEKS
Kamu adalah Ahli Kurikulum dan Pengembang Perangkat Ajar berpengalaman di satuan pendidikan naungan Kemendikbudristek RI, menguasai penyusunan Modul Ajar berbasis Deep Learning (Mindful, Meaningful, Joyful Learning) yang diintegrasikan penuh dengan Kurikulum Berbasis Merdeka (KBM) — Dimensi Profil Pelajar Pancasila (P3) & Nilai Karakter Universal.

TUGAS
Susun dokumen "PERENCANAAN PEMBELAJARAN (MODUL AJAR DEEP LEARNING - KBM)" untuk Tujuan Pembelajaran [Kode: ${kodeTp}, Rumusan: ${rumusanTp}] mata pelajaran ${subject} kelas ${level} tahun pelajaran ${year}, dengan identitas satuan pendidikan:
- Kemendikbudristek Republik Indonesia
- ${kemenagOffice}
- ${schoolName}
- ${schoolAddress}
- Penyusun: ${teacher} (NIP: ${nipTeacher}) | Kepala Sekolah: ${principal} (NIP: ${nipPrincipal})
...
KOP SURAT DOKUMEN:
PEMERINTAH KOTA TARAKAN / DINAS PENDIDIKAN
...
- 1. Tujuan Pembelajaran (1 kalimat utuh memuat kompetensi + nilai KBM yang ditumbuhkan)
- 2. Kompetensi Awal/Prasyarat (tabel: No | Kompetensi Prasyarat | Cara Mengecek)
- 3. 8 Dimensi Profil Lulusan (pilih 2 dimensi paling relevan; tabel: No | Dimensi | Deskripsi Perwujudan, setiap deskripsi WAJIB diakhiri anotasi "(P3): [Dimensi Profil Pelajar Pancasila] | NKU: [Nilai Karakter Universal])")
- 3a. Integrasi KBM: baris terpisah "Dimensi Profil Pelajar Pancasila (P3): [...]" dan "Nilai Karakter Universal: [...]" (pilih 2-3 nilai paling relevan dari 10 Nilai Karakter Universal)
...
- 6. Pemahaman Bermakna (kotak bertekstur naratif-reflektif, mengaitkan materi dengan minimal 1 istilah NKU, misal Pikiran Terbuka (Open-Mindedness) / Kepemimpinan Berkarakter (Leadership) / Etika & Moralitas)
...
- 9. KEGIATAN PEMBELAJARAN — cantumkan nama model (${learningModel}) & urutan sintak lengkap (${sintakModel}) di awal.
  ...
  c. ● PEMBUKA (15 menit) — prinsip Deep Learning + integrasi KBM (P3 & NKU), tabel 2 kolom (Aktivitas Guru | Aktivitas Siswa) berpasangan No, 5 baris, SETIAP baris aktivitas guru MAUPUN siswa diakhiri anotasi "(P3: ... | NKU: ...)"
...
- 13. Refleksi Guru & Peserta Didik (2 kolom, masing-masing 3-4 pertanyaan reflektif, refleksi guru memuat kata "teladan")
...
- Blok tanda tangan: ${cityDate} | Mengetahui Kepala Sekolah (${principal}, NIP: ${nipPrincipal}) — Dibuat oleh Guru Mata Pelajaran (${teacher}, NIP: ${nipTeacher}).
Layout: A4 Portrait (@media print { @page { size: A4 portrait; margin: 1.2cm; } }).
`;
...
PERAN & KONTEKS
Kamu adalah Ahli Kurikulum dan Pengembang Perangkat Ajar berpengalaman di satuan pendidikan naungan Kemendikbudristek RI, menguasai penyusunan LKPD berbasis Deep Learning (Mindful, Meaningful, Joyful Learning) yang diintegrasikan penuh dengan Kurikulum Berbasis Merdeka (KBM) — Dimensi Profil Pelajar Pancasila (P3) & Nilai Karakter Universal.

TUGAS
Susun dokumen "LEMBAR KERJA PESERTA DIDIK (LKPD) KURIKULUM BERBASIS MERDEKA" untuk Tujuan Pembelajaran [Kode: ${kodeTp}, Rumusan: ${rumusanTp}] mata pelajaran ${subject} kelas ${level} tahun pelajaran ${year}, dengan identitas satuan pendidikan:
- Kemendikbudristek Republik Indonesia
- ${kemenagOffice}
- ${schoolName}
- ${schoolAddress}
...
- Kop surat identik dengan Modul Ajar:
  ...
  ${schoolName.toUpperCase()}
  ${schoolAddress}
- Judul LKPD + subjudul tematik (mis. "Lembar Penilaian Sahabat & Eksplorasi ${topikLokal}")
...
- Lembar Penilaian Sahabat di akhir jika ada nilai kolaborasi: tabel indikator (4 butir) dengan 3 kolom checklist (Sangat Baik | Baik | Perlu Bimbingan)
- Blok tanda tangan: ${cityDate} | Mengetahui Kepala Sekolah (${principal}, NIP: ${nipPrincipal}) — Dibuat oleh Guru Mata Pelajaran (${teacher}, NIP: ${nipTeacher}).
Layout: A4 Portrait (@media print { @page { size: A4 portrait; margin: 1.2cm; } }).
`;
...
TUGAS
Susun dokumen "RUBRIK PENILAIAN FORMATIF & SUMATIF KURIKULUM BERBASIS MERDEKA" untuk Tujuan Pembelajaran [Kode: ${kodeTp}, Rumusan: ${rumusanTp}] mata pelajaran ${subject} kelas ${level} tahun pelajaran ${year}, dengan identitas satuan pendidikan:
- Kemendikbudristek Republik Indonesia
- ${kemenagOffice}
- ${schoolName}
- ${schoolAddress}
...
- Kop surat identik dengan Modul Ajar + judul "RUBRIK PENILAIAN FORMATIF KURIKULUM BERBASIS MERDEKA" + rumusan TP (${kodeTp}: ${rumusanTp}) + metadata (Mapel: ${subject} | Fase/Kelas: ${level} | Tahun: ${year} | Model: ${learningModel} | Nama Guru: ${teacher})
- Per pertemuan (Pertemuan 1 s.d. ${jumlahPertemuan}): judul pertemuan + sintak, lalu tabel 5 kolom (Kriteria | 4-Sangat Berkembang | 3-BSH | 2-Mulai Berkembang | 1-Perlu Bimbingan), 3 kriteria kinerja per pertemuan selaras dengan Tujuan Pertemuan pada Modul Ajar
  - Kalimat level 4 SELALU pola: "[capaian superlatif], menjadi teladan [nilai]."
...
- Di pertemuan terakhir, tambahkan:
  ...
  C. Penilaian Integrasi Nilai (KBM P3 & NKU) — kriteria = Dimensi Profil Pelajar Pancasila (P3) & Nilai Karakter Universal terpilih
- "RUBRIK PENILAIAN SUMATIF KURIKULUM BERBASIS MERDEKA" — judul TP sama + metadata, lalu Komponen Penilaian (list persentase, total 100%) dan tabel rubrik 6 kolom (No | Kriteria | 4 | 3 | 2/1 digabung | Bobot), Total Bobot 100%
- Blok tanda tangan: ${cityDate} | Mengetahui Kepala Sekolah (${principal}, NIP: ${nipPrincipal}) — Dibuat oleh Guru Mata Pelajaran (${teacher}, NIP: ${nipTeacher}).
Layout: A4 Landscape (@media print { @page { size: A4 landscape; margin: 1.0cm; } }).
`;
      } else {
      ...
      console.error("Gagal generate Perangkat Ajar KBM:", err);
      res.status(500).json({
        status: "error",
        message: err?.message || "Terjadi kesalahan saat memproses Perangkat Ajar KBM."
      });
    }
  });
  ...
  app.post("/api/ai/generate-kartu-soal", async (req, res) => {
    try {
      ...
      const school = formData?.school || "SMP NEGERI 10 TARAKAN";
      const subject = formData?.subject || "Bimbingan Konseling";
      const level = formData?.level || "Kelas VIII / Fase D";
      const curriculum = formData?.curriculum || "Kurikulum Merdeka";
      const mainTopic = formData?.mainTopic || "Laporan Hasil Observasi Siswa (Studi Kasus/Asesmen)";
...
`;
  ...
  app.post("/api/ai/generate-modul-kokurikuler", async (req, res) => {
    try {
      ...
      const topic = formData?.topic || "Suara Demokrasi dan Anti-Bullying: Mewujudkan Sekolah Ramah Anak";
      const duration = formData?.duration || "3 JP (4 x 40 Menit)";
      const location = formData?.location || "Lingkungan Taman Sekolah & Gazebo Adiwiyata Sekolah";
      const selectedDimensions = Array.isArray(formData?.dimensions) && formData.dimensions.length > 0 
        ? formData.dimensions 
        : ["Penalaran Kritis", "Kolaborasi"];
      const subject1 = formData?.subject1 || "Bimbingan Konseling";
      const subject2 = formData?.subject2 || "PPKn";
      const pedagogicalPractice = formData?.pedagogicalPractice || "Pembelajaran Berbasis Proyek (Project-Based Learning)";
      const learningEnvironment = formData?.learningEnvironment || "Memberi kesempatan siswa untuk mengeksplorasi secara nyata iklim sosial dan kasus perundungan (bullying) di lingkungan sekolah, serta mempraktikkan proses pembuatan sistem pengaduan aman dan kampanye mediasi sebaya";
      const partnership = formData?.partnership || "Kolaborasi lintas mapel PKn (analisis pelanggaran hak asasi manusia, norma hukum, dan regulasi sekolah) dan Bimbingan Konseling (psikoedukasi kesehatan mental, penumbuhan empati, dan mekanisme konseling sebaya), serta pendampingan dari Komisi Perlindungan Anak/Psikolog Anak";
      const digitalTools = formData?.digitalTools || "Canva untuk infografis kampanye anti-perundungan, Google Forms untuk pembuatan platform pengaduan atau survei kesejahteraan psikologis (well-being) siswa secara anonim, dan podcast/video edukasi stop bullying";
...
`;
  ...
  app.post("/api/ai/generate-soal-ujian", async (req, res) => {
    try {
      ...
      const logoUrl = formData?.logoUrl || "https://lh3.googleusercontent.com/d/1i82llPU1_OoXCVIVQKOHVrRNkV1Cc5Ho";
      const teacher = formData?.teacher || "Al-Kahpi, S.Pd.";
      const nipTeacher = formData?.nipTeacher || "198208242011011001";
      const principal = formData?.principal || "IBRAHIM, M.Pd.";
      const nipPrincipal = formData?.nipPrincipal || "197802152008011021";
      const cityDate = formData?.cityDate || "Tarakan, 1 Desember 2026";
