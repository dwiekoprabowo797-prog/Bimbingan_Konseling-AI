...
export const GeneratorPerangkatAjarAIView: React.FC<GeneratorPerangkatAjarAIViewProps> = ({ config }) => {
  ...
  const [formData, setFormData] = useState({
    school: config.Nama_Sekolah || "SMP Negeri 10 Tarakan",
    subject: "Bimbingan Konseling",
    singkatanMapel: "BK",
    level: "Fase D / Kelas VII",
    year: "2026/2027",
    totalJp: "72 JP / Tahun",
    ...
    cpRasional: "Pada akhir Fase D, peserta didik mampu mengenali, mengelola, dan mengembangkan potensi diri secara optimal dalam dimensi pribadi, sosial, belajar, dan karir sesuai dengan Standar Kompetensi Kemandirian Peserta Didik (SKKPD). Peserta didik memiliki kesadaran kesehatan mental, etika berinteraksi di dunia nyata maupun digital, keterampilan regulasi emosi, serta mumpuni dalam merencanakan studi dan masa depan secara rasional, mandiri, dan bertanggung jawab.",
    cpElemen: `Elemen 1 — Pribadi (Pengembangan Diri & Kesehatan Mental):
Peserta didik mampu memahami kelebihan dan keterbatasan diri, mengelola emosi secara positif, membangun motivasi internal, serta mengaplikasikan prinsip-prinsip kesehatan mental dan etika digital (digital hygiene) dalam kehidupan sehari-hari guna membentuk pribadi yang tangguh (resilient) dan berkarakter.

Elemen 2 — Sosial (Etika Interaksi & Empati Digital):
Peserta didik mampu membangun interaksi sosial yang harmonis dan inklusif, menerapkan empati serta komunikasi efektif, mengatasi tekanan teman sebaya (peer pressure), mencegah dan menangani tindakan perundungan (bullying/cyberbullying), serta mengelola konflik interpersonal secara damai dan konstruktif.

Elemen 3 — Belajar (Agilitas Belajar & Manajemen Diri):
Peserta didik mampu mengidentifikasi gaya belajar pribadi, mengoreksi perilaku prokrastinasi (menunda pekerjaan), merancang strategi manajemen waktu yang efektif, serta mengelola kecemasan akademik (academic anxiety) untuk mencapai efikasi diri dan prestasi belajar yang optimal.

Elemen 4 — Karir (Eksplorasi Potensi & Perencanaan Masa Depan):
Peserta didik mampu mengeplorasi bakat, minat, dan nilai-nilai karir, memahami peluang profesi baru di era digital dan Artificial Intelligence (AI), serta menyusun perencanaan awal studi lanjutan (pemilihan jalur SMA/SMK) yang selaras dengan potensi dan cita-cita personal.`
  });
...
Peserta didik mampu mengeplorasi bakat, minat, dan nilai-nilai karir, memahami peluang profesi baru di era digital dan Artificial Intelligence (AI), serta menyusun perencanaan awal studi lanjutan (pemilihan jalur SMA/SMK) yang selaras dengan potensi dan cita-cita personal.`
    ...
    notifySimpanSuccess("Contoh data Bimbingan Konseling Kelas VII berhasil dimuat!");
  };
  ...
  const sanitizeHtmlForOutput = (rawHtml: string) => {
    if (!rawHtml) return "";
    let clean = rawHtml;
    const defaultLogoTag = `<img src="${config.Logo_Kiri || config.Logo_Kanan || 'https://lh3.googleusercontent.com/d/1i82llPU1_OoXCVIVQKOHVrRNkV1Cc5Ho'}" alt="Logo Sekolah" class="logo-sekolah" style="max-height: 70px; width: auto; float: left; margin-right: 15px; object-fit: contain;" />`;
  ...
  const handlePrintA4 = () => {
    ...
    try {
      ...
      console.warn("Could not open print window:", e);
    }
    if (!printWindow) {
