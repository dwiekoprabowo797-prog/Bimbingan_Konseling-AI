...
export const PerangkatAjarKBCView: React.FC<PerangkatAjarKBCViewProps> = ({ config }) => {
  ...
  const [formData, setFormData] = useState({
    schoolName: config.Nama_Sekolah || "SMP NEGERI 10 TARAKAN",
    ...
    teacher: config.Nama_Guru || "Al-Kahpi, S.Pd.",
    nipTeacher: config.NIP_Guru || "198208242011011001",
    cityDate: `${config.Tempat_Tanda_Tangan || "Tarakan"}, 14 Juli 2026`,
    principal: config.Nama_Kepsek || "IBRAHIM, M.Pd.",
    nipPrincipal: config.NIP_Kepsek || "197802152008011021",
    learningModel: "Discovery Learning",
    cpRasional: "Layanan Bimbingan dan Konseling (BK) di SMPN 10 Tarakan mengintegrasikan 6 Dimensi Profil Pelajar Pancasila (P3) serta Kearifan Lokal Pesisir Berbudaya Kota Tarakan bukan sebagai materi hafalan teoretis, melainkan sebagai navigasi psikologis dan kesadaran emosional. BK mentransformasikan dimensi karakter tersebut ke dalam kemandirian perilaku, kesehatan mental, dan ketahanan sosial-emosional peserta didik di lingkungan pesisir yang heterogen. Melalui pendekatan psikologis yang humanis dan kolaboratif, layanan BK memfasilitasi peserta didik agar mampu mengelola gejolak perkembangan remaja, menumbuhkan empati anti-perundungan lintas suku, serta mengambil keputusan karier masa depan secara bijak demi terwujudnya pribadi yang mandiri, bernalar kritis, dan berakhlak mulia.",
    cpElemen: `Elemen 1 — Bimbingan Pribadi (Pengembangan Diri & Regulasi Emosi):
Peserta didik mampu mengenali, menerima, dan mengelola perubahan fisik serta psikologis masa pubertas secara sehat. Peserta didik menginternalisasi dimensi Beriman, Bertakwa kepada Tuhan YME, Berakhlak Mulia, dan Mandiri melalui pembiasaan perilaku santun, regulasi emosi yang positif, serta kesadaran menjaga kebersihan diri dan lingkungan pesisir Tarakan, sehingga mampu membangun kepercayaan diri serta menjaga integritas moral dari perilaku merusak diri.

Elemen 2 — Bimbingan Sosial (Hubungan Interpersonal & Kewarganegaraan):
Peserta didik mampu membangun komunikasi asertif, empati, dan kerja sama yang harmonis dengan teman sebaya, keluarga, serta masyarakat. Peserta didik menerapkan dimensi Berkebinekaan Global dan Gotong Royong yang diwujudkan melalui sikap menghargai keberagaman suku dan budaya heterogen di Kota Tarakan, saling peduli, dan berkolaborasi untuk mencegah segala bentuk perundungan (bullying/cyberbullying) serta menyelesaikan konflik sosial secara damai.

Elemen 3 — Bimbingan Belajar (Manajemen Akademik & Daya Juang):
Peserta didik mampu memetakan gaya belajar pribadi, mengatasi kejenuhan (burnout), dan mengelola waktu belajar secara efektif. Peserta didik menginternalisasi dimensi Bernalar Kritis melalui prinsip keseimbangan waktu, sehingga memiliki kemandirian belajar, daya juang akademik (resilience), serta bijak dan bertanggung jawab dalam memanfaatkan teknologi digital untuk mendukung prestasi belajar.

Elemen 4 — Bimbingan Karier (Eksplorasi Potensi & Orientasi Masa Depan):
Peserta didik mampu mengeksplorasi minat, bakat, kelebihan, dan keterbatasan diri untuk merencanakan kelanjutan studi atau arah masa depannya. Peserta didik mengintegrasikan dimensi Kreatif dan semangat cinta tanah air, sehingga siap menjadi generasi produktif yang adaptif terhadap perkembangan zaman, jeli melihat peluang karier di kota pelabuhan/industri Tarakan, serta berkontribusi bagi kemaslahatan lingkungan sekitar.`
  });
  ...
  const docTypeList = [
    { id: "analisis_cp", label: "1. Analisis CP", fullTitle: "Analisis Capaian Pembelajaran (ACP) KBM", icon: BookOpen },
    { id: "tp", label: "2. Tujuan Pembelajaran", fullTitle: "Tujuan Pembelajaran (TP) KBM", icon: FileCheck },
    { id: "atp", label: "3. Alur TP (ATP)", fullTitle: "Alur Tujuan Pembelajaran (ATP) KBM", icon: Layers },
    { id: "prota", label: "4. Program Tahunan", fullTitle: "Program Tahunan (Prota) KBM", icon: Calendar },
    { id: "prosem", label: "5. Program Semester", fullTitle: "Program Semester (Prosem) KBM", icon: FileSpreadsheet },
    { id: "kktp", label: "6. KKTP KBM", fullTitle: "Kriteria Ketercapaian Tujuan Pembelajaran (KKTP) KBM", icon: Award },
    { id: "modul_ajar", label: "7. Modul Ajar KBM", fullTitle: "Modul Ajar Deep Learning KBM", icon: HeartHandshake },
    { id: "lkpd", label: "8. LKPD KBM", fullTitle: "Lembar Kerja Peserta Didik (LKPD) KBM", icon: FileText },
    { id: "rubrik", label: "9. Rubrik Formatif & Sumatif", fullTitle: "Rubrik Penilaian Formatif & Sumatif KBM", icon: CheckSquare }
  ];
...
Peserta didik mampu mengeksplorasi minat, bakat, kelebihan, dan keterbatasan diri untuk merencanakan kelanjutan studi atau arah masa depannya. Peserta didik mengintegrasikan dimensi Kreatif dan semangat cinta tanah air, sehingga siap menjadi generasi produktif yang adaptif terhadap perkembangan zaman, jeli melihat peluang karier di kota pelabuhan/industri Tarakan, serta berkontribusi bagi kemaslahatan lingkungan sekitar.`
    ...
    notifySimpanSuccess("Contoh data Administrasi KBM berhasil dimuat!");
  };

  const handleFillSampleModul = () => {
    setFormDataModul({
      kemenagOffice: "PEMERINTAH KOTA TARAKAN",
      schoolName: config.Nama_Sekolah || "PEMERINTAH KOTA TARAKAN",
      schoolAddress: "Jl. Amal Lama RT. 4 Kel. Pantai Amal Kec. Tarakan Timur Kota Tarakan",
      subject: "Bimbingan Konseling",
      level: "Fase D / Kelas VIII",
      year: "2026/2027",
    ...
    notifySimpanSuccess("Contoh data Khusus Modul Ajar, LKPD & Rubrik KBM berhasil dimuat!");
  };

  const sanitizeHtmlForOutput = (rawHtml: string) => {
    if (!rawHtml) return "";
    let clean = rawHtml;
    const defaultLogoTag = `<img src="${config.Logo_Kiri || config.Logo_Kanan || 'https://lh3.googleusercontent.com/d/1i82llPU1_OoXCVIVQKOHVrRNkV1Cc5Ho'}" alt="Logo Kemenag/Sekolah" class="logo-sekolah" style="max-height: 70px; width: auto; float: left; margin-right: 15px; object-fit: contain;" />`;
  ...
  const handleGenerate3ModulDocs = async () => {
    setIsGenerating(true);
    const modulTypes = [
      { id: "modul_ajar", title: "Modul Ajar Deep Learning KBM" },
      { id: "lkpd", title: "LKPD KBM" },
      { id: "rubrik", title: "Rubrik Penilaian Formatif & Sumatif KBM" }
    ];
    ...
    notifySimpanSuccess("Paket 3 Dokumen (Modul Ajar, LKPD, & Rubrik KBM) Berhasil Dibuat!");
  };
  ...
  const handleGenerateAllDocs = async () => {
    ...
    setIsGenerating(false);
    notifySimpanSuccess("Seluruh 9 Dokumen Administrasi Perangkat Ajar KBM Berhasil Dibuat!");
  };
  ...
  const handlePrintA4 = () => {
    ...
    if (!rawHtml) {
      notifySimpanError("Belum ada dokumen KBM yang dihasilkan untuk dicetak.");
      return;
    }
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Cetak Perangkat Ajar KBM - A4 Presisi</title>
          <meta charset="utf-8" />
          <style>
            @page {
  ...
  const handleDownloadWord = () => {
    const rawHtml = generatedDocs[activeDoc];
    if (!rawHtml) {
      notifySimpanError("Belum ada dokumen KBM yang dihasilkan untuk diunduh.");
      return;
    }

    const activeHtml = sanitizeHtmlForOutput(rawHtml);
    const orientation = activeDoc === "atp" || activeDoc === "prosem" || activeDoc === "kktp" || activeDoc === "rubrik" ? "landscape" : "portrait";
    const docTitle = docTypeList.find((d) => d.id === activeDoc)?.fullTitle || "Perangkat_Ajar_KBM";
    ...
    a.href = url;
    a.download = `${activeDoc.toUpperCase()}_KBM_${currentSubject.replace(/\s+/g, "_")}.doc`;
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center space-x-2 bg-emerald-400 text-slate-950 font-black px-3 py-1 rounded-full text-xs uppercase tracking-wider shadow-xs">
              ...
              <span>Kurikulum Berbasis Merdeka (KBM) Dinas Pendidikan</span>
            </div>
            <h2 className="text-2xl md:text-3xl font-black tracking-tight text-emerald-300">
              Perangkat Ajar KBM AI
            </h2>
            <p className="text-slate-200 text-xs md:text-sm leading-relaxed">
              Generator 9 Paket Perangkat Administrasi Pembelajaran KBM (Analisis CP, TP, ATP, Prota, Prosem, KKTP, Modul Ajar Deep Learning, LKPD, & Rubrik Formatif/Sumatif) terintegrasi Terintegrasi Profil Pelajar Pancasila (P3) dan Karakter Pesisir Berbudaya (Muatan Lokal Kota Tarakan) untuk SMPN 10 Tarakan!
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 shrink-0 w-full md:w-auto">
            {inputTab === "modul" ? (
              ...
              >
                <Sparkles className="w-4 h-4" />
                <span>Generate 9 Dokumen KBM</span>
              </button>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800">
        ...
        >
          ...
          <span>1. Input Administrasi KBM (ACP, TP, ATP, Prota, Prosem, KKTP)</span>
        </button>
        ...
        >
          ...
          <span>2. Input Khusus Modul Ajar, LKPD & Rubrik KBM</span>
        </button>
      </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-md space-y-6">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-3 flex-wrap gap-2">
            <h3 className="text-base md:text-lg font-black text-slate-800 dark:text-slate-100 flex items-center space-x-2">
              ...
              <span>Data Identitas Sekolah & Capaian Pembelajaran (Dokumen Administrasi 1-6)</span>
            </h3>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
              <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Kantor / Yayasan</label>
              <input
                type="text"
            ...
            <div>
              <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Satuan Pendidikan / Sekolah</label>
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-2">
            <div>
              <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">
                CP Umum / Rasional KBM (Profil Pelajar Pancasila (P3) dan Muatan Lokal Pesisir Berbudaya Kota Tarakan)
              </label>
              <textarea
                rows={4}
      ...
      {/* Form Input Mode 2: KHUSUS Modul Ajar, LKPD, & Rubrik KBM */}
      {inputTab === "modul" && (
        <div className="bg-emerald-50/50 dark:bg-emerald-950/20 rounded-2xl p-6 border-2 border-emerald-300 dark:border-emerald-800 shadow-lg space-y-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <h3 className="text-base md:text-lg font-black text-emerald-950 dark:text-emerald-200">
                  Input Khusus: Modul Ajar Deep Learning, LKPD, & Rubrik KBM
                </h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs bg-white dark:bg-slate-800 p-4 rounded-xl border border-emerald-200 dark:border-emerald-900">
              ...
              <div className="md:col-span-3">
                ...
                <textarea
                  ...
                  placeholder="misal: Pelestarian Ekosistem Mangrove & Bekantan (KKMB), Harmoni Remaja Multietnis (Tidung, Dayak, Bugis, Jawa), serta Filosofi Kerja Sama Budaya Iraw Tengkayu Kota Tarakan...."
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs bg-white dark:bg-slate-800 p-4 rounded-xl border border-emerald-200 dark:border-emerald-900">
              <div>
                <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Kantor Dinas Pendidikan/Kota</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Nama Sekolah / Yayasan</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-bold text-slate-700 dark:text-slate-300 block mb-1">Alamat Lengkap Sekolah</label>
                <input
                  type="text"
        ...
        <div className="flex items-center justify-between flex-wrap gap-3 border-b border-slate-200 dark:border-slate-700 pb-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-extrabold text-slate-800 dark:text-slate-100 text-sm md:text-base">
              Pilih Dokumen KBM Yang Ingin Digenerate / Ditampilkan:
            </h3>
          </div>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 p-6 rounded-2xl flex items-center justify-center space-x-4 animate-pulse">
          ...
          <div>
            <p className="font-extrabold text-emerald-900 dark:text-emerald-200 text-sm md:text-base">
              {generatingProgress || "Sedang memproses AI Perangkat Ajar KBM..."}
            </p>
          </div>
        </div>
      )}
